...
public class Generator {
    ...
    public TerrainChunk[] generateTerrain(int size, int chunkSize, int radius, float noiseFrequency, float noiseMagnitude){
        ...
        float c=(float)chunksNumber/2;
        int count=0;
        for(float x=-c;x<c;x++){
            for(float y=-c;y<c;y++){
                for(float z=-c;z<c;z++){
                    float[][][] localField=genScalarField(x*chunkSize,y*chunkSize,z*chunkSize,chunkSize+1,radius,noiseFrequency,noiseMagnitude);
                    chunks[count]=new TerrainChunk(new Vector3f(x*chunkSize,y*chunkSize,z*chunkSize),localField,this);
                    count++;
                }
            }
        }
    ...
    private float generateScalar(float x, float y, float z, int radius, float noiseFrequency, float noiseMagnitude){
        float distFromOrigin=(float)Math.sqrt(Math.abs(square(x)+square(y)+square(z)));

        return (radius-distFromOrigin)+(noise.genPoint(x*noiseFrequency,y*noiseFrequency,z*noiseFrequency)-0.95f)*noiseMagnitude;
    }

    private float[][][] genScalarField(float offsetX, float offsetY, float offsetZ,int size, int radius, float noiseFrequency, float noiseMagnitude){
        float[][][] field=new float[size][size][size];
        for(int x=0;x<size;x++){
            for(int y=0;y<size;y++){
                for(int z=0;z<size;z++){
                    field[x][y][z]=generateScalar(x+offsetX+0.1f,y+offsetY+0.1f,z+offsetZ+0.1f,radius,noiseFrequency,noiseMagnitude);
                }
            }
        }
        return field;
